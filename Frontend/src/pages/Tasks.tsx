...
export function TasksPage() {
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (state.editing && selectedTask) {
      await axios.put(`${apiURL}/${id}`, newTask);
  ...
  const filterTasks = useCallback((tasks: Task[], filter: string) => {
  ...
  const renderTasks = useCallback(() => {
    ...
    if (state.filterTasks.length > 0) {
      return state.filterTasks.map((task: Task) => {
        return (
          <Task
            title={task.title}
  ...
  return (
    <div className="max-w-[1200px] max-h-[var(--mainDiv-h)] mx-auto p-8 md:grid md:grid-cols-[300px_1fr] gap-8 flex flex-col overflow-y-scroll md:overflow-hidden [&::-webkit-scrollbar-track]:bg-white/20  [&::-webkit-scrollbar-thumb]:bg-white/20 [&::-webkit-scrollbar-thumb]:rounded-xs [&::-webkit-scrollbar]:w-3 [&::-webkit-scrollbar]:h-2 [&::-webkit-scrollbar-track]:rounded-xs">
      ...
      /*<div className="max-h-[596px] bg-white/10 rounded-2xl p-6 shadow-md backdrop-blur-md md:sticky top-8 border border-white/10">
        ...
        <div className="flex flex-col mt-[0.7rem] border-t-white/10 border-t-1">
          <h3 className="text-primary-ta mt-[0.7rem] mb-2 text-[1.17rem] font-bold">
            Filter Tasks
          </h3>
